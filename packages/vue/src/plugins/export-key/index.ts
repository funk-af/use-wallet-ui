...
import ExportKeyDialog from './ExportKeyDialog.vue'
import ExportKeyMenuButton from './ExportKeyMenuButton.vue'
    ...
    menuItems: [
      {
        ...
        order: 10,
        enabled: (ctx) =>
          !!(ctx.activeWallet as { canUsePrivateKey?: boolean })
            ?.canUsePrivateKey,
        render: (ctx) => h(ExportKeyMenuButton, { ctx }),
      },
    ],
    ...
    dialogs: [
      {
        ...
        render: ({ onClose }) =>
          h(ExportKeyDialog, { onClose, displayTimeout }),
      },
    ],
  }
}
